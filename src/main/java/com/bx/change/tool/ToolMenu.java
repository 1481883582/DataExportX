...
enum ToolMenu {
    ...
    ENCODE("编/解码", null),
    ENCRYPT("加/解密", null),
    SQL_CHANGE("Sql转换", null),

    /**
     * ENCODE  解码子下拉框  sub menu
     */
    ...
    SQl_MYSQL("Mysql库", ConvertType.TEXT) {
        @Override
        void handle(RSyntaxTextArea input, RSyntaxTextArea output) {
            output.setText(new String(input.getText().getBytes()));
        }
    },
    SQl_OSRDB("神通数据库", ConvertType.TEXT) {
        @Override
        void handle(RSyntaxTextArea input, RSyntaxTextArea output) {
            output.setText(new String(input.getText().getBytes()));
        }
